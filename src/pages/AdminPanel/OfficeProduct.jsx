...
const OfficeProduct = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Refresh images for the selected product
      await fetchImages(selectedProduct.id);
  
      // Finally, reset the selected product (if you want)
    } catch (error) {
    ...
    <div className='min-h-screen bg-gray-100 flex flex-col items-center pt-10'>
      ...
      <div className='container max-w-4xl bg-white p-8 rounded-lg shadow-lg'>
        ...
        <div className='mt-10 flex justify-center md:justify-start space-x-3'>
          <Link to={"/admin"}><button className='px-5 py-1 border rounded-lg text-white bg-green-500'>Previous</button></Link>
          <Link to={"/admin"}><button className='px-5 py-1 border rounded-lg text-white bg-green-500'>Submit</button></Link>
        </div>
      </div>
    </div>
  );
