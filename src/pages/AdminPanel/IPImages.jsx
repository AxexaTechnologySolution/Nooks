...
const IPImages = () => {
  ...
  const fetchCategories = async (productId) => {
    const categoryCollection = collection(db, `institutionalProducts/${productId}/images/${selectedCategory}`);
  ...
  const fetchUploadedImages = async (categoryId) => {
    // const imagesCollection = collection(db, `institutionalProducts/${selectedProduct}/categories/${categoryId}/images`);
    const imagesCollection = collection(db, `institutionalProducts/${selectedProduct}/images/${categoryId}/categories`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const imageRef = ref(storage, `institutional/${imageFile.name}`);
      await uploadBytes(imageRef, imageFile);
      const imageUrl = await getDownloadURL(imageRef);

      // Store image details in Firestore under the selected product and category
      const imageDocRef = doc(collection(db, `institutionalProducts/${selectedProduct}/images/${selectedCategory}/categories`));
  ...
  const handleDelete = async (imageId) => {
    if (window.confirm("Are you sure you want to delete this image?")) {
      try {
        // Correct path for deleting the image
        await deleteDoc(doc(db, `institutionalProducts/${selectedProduct}/images/${selectedCategory}/categories`, imageId));
  ...
  return (
    <div className='min-h-screen bg-gray-100 flex flex-col items-center pt-10'>
      ...
      <div className='container max-w-4xl bg-white p-8 rounded-lg shadow-lg'>
        <h1 className="md:text-4xl text-2xl font-extrabold text-gray-800 mb-10">Institutional Product Images</h1>
        ...
        <div className='flex flex-col space-y-6'>
          ...
          <button
            ...
            className={`bg-green-600 text-white p-3 rounded-lg flex items-center justify-center shadow-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 ${isSubmitting ? "opacity-50 cursor-not-allowed" : ""}`}
